...
function IrisLens(){
  ...
  const generate=useCallback(async(dataUrl,colorLabel)=>{
    ...
    try{
      ...
      const item={url:output,color:finalColor,date:new Date().toLocaleDateString()};
    ...
    <div style={{minHeight:"100vh",background:"#000",color:"#fff",fontFamily:"'Inter', system-ui, sans-serif",overflowX:"hidden"}}>
      ...
      {screen==="home"&&(
        <div style={{height:"100vh",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden",background:"#000"}}>
          {/* Background image — treated as atmospheric texture */}
          <div style={{position:"absolute",inset:0}}>
            ...
            {/* Radial vignette — darker at edges, slightly lighter at face */}
            ...
            {/* Horizontal gradient — left side even darker for text legibility */}
          ...
          {/* Main content — pushed to lower portion of screen */}
          <div style={{position:"relative",zIndex:2,marginTop:"auto",padding:"0 36px 56px",animation:"fadeUp 1s cubic-bezier(0.16,1,0.3,1) forwards"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
              ...
              <span className="label" style={{color:C,opacity:.9}}>AI Iris Portrait</span>
            </div>

            {/* Display headline */}
            <h1 className="display" style={{fontSize:"clamp(52px,11vw,80px)",margin:"0 0 4px",color:"#fff"}}>Reveal your</h1>
            <h1 className="display" style={{fontSize:"clamp(52px,11vw,80px)",margin:"0 0 36px",color:"rgba(255,255,255,.55)",fontStyle:"italic"}}>iris.</h1>
        ...
        <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 36px",textAlign:"center",background:"#000"}}>
          {!genError?(
            <>
              ...
              <span className="label" style={{color:"rgba(255,255,255,.2)"}}>AI rendering — up to 30 seconds</span>
            </>
          ):(
            <>
        ...
        <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"space-between",background:"#000",padding:"0 24px"}}>
          ...
          <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",width:"100%",padding:"16px 0"}}>
            <div style={{position:"relative"}}>
              ...
              <img
                src={result.url} alt="Your iris portrait"
                style={{
          ...
          {/* Footer: colour + actions */}
          <div style={{width:"100%",maxWidth:420,paddingBottom:"max(52px,env(safe-area-inset-bottom))",textAlign:"center"}}>
            {/* Colour */}
        ...
        <div style={{minHeight:"100vh",background:"#000",padding:"36px 28px"}}>
          ...
          {gallery.length===0?(
            <div style={{textAlign:"center",paddingTop:80}}>
              ...
              <button className="btn-cyan" onClick={()=>setShowExplainer(true)}>Begin Capture</button>
            </div>
          ):(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:20}}>
              {gallery.map((it,i)=>(
                <div key={i} onClick={()=>setExpanded(it)} style={{cursor:"pointer",textAlign:"center"}}>
                  ...
                  <span style={{fontSize:9,color:"rgba(255,255,255,.15)",display:"block",marginTop:2}}>{it.date}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {/* ── EXPANDED ── */}
      {expanded&&(
        <div onClick={()=>setExpanded(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.97)",display:"flex",alignItems:"center",justifyContent:"center",padding:24,zIndex:30}}>
          <img src={expanded.url} alt="" style={{maxWidth:"90vw",maxHeight:"80vh",borderRadius:"50%",boxShadow:"0 0 0 1px rgba(255,255,255,.06)"}}/>
        </div>
      )}
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",backdropFilter:"blur(16px)",display:"flex",alignItems:"center",justifyContent:"center",padding:24,zIndex:40}}>
          <div style={{background:"#0a0a0a",border:"1px solid rgba(255,255,255,.08)",padding:"44px 36px",maxWidth:320,textAlign:"center",width:"100%"}}>
            ...
            <p style={{fontSize:13,fontWeight:300,lineHeight:1.8,color:"rgba(255,255,255,.45)",margin:"0 0 36px"}}>
              IrisLens uses your camera to guide the capture. Your image is processed on your device — nothing is uploaded.
            </p>
