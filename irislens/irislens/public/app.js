var IrisLensBundle = (() => {
  ...
  function IrisLens() {
    ...
    const loadLandmarker = useCallback(async () => {
      ...
      // Try GPU first (much faster). Fall back to CPU if GPU init fails,
      // which happens on some iOS Safari and older Android WebView builds.
      let lm;
      try {
        lm = await FaceLandmarker.createFromOptions(fs, { baseOptions: { modelAssetPath: MODEL_URL, delegate: "GPU" }, runningMode: "VIDEO", numFaces: 1, outputFaceBlendshapes: false });
        if (window.__irisDebug) console.log("[mp] GPU delegate ok");
      } catch (gpuErr) {
        console.warn("[mp] GPU delegate failed, falling back to CPU:", gpuErr?.message || gpuErr);
        lm = await FaceLandmarker.createFromOptions(fs, { baseOptions: { modelAssetPath: MODEL_URL, delegate: "CPU" }, runningMode: "VIDEO", numFaces: 1, outputFaceBlendshapes: false });
      }
    ...
    const startCamera = useCallback(async () => {
      ...
      try {
        ...
        if (window.__irisDebug && videoRef.current) {
          const v = videoRef.current;
          const s = stream.getVideoTracks()[0]?.getSettings?.() || {};
          console.log(`[cam] video=${v.videoWidth}x${v.videoHeight} track=${s.width}x${s.height} facing=${s.facingMode} screen=${window.innerWidth}x${window.innerHeight} dpr=${window.devicePixelRatio}`);
        }
    ...
    const measureExposure = (video, ncx, ncy, nrad) => {
      ...
      if (!exposureCanvasRef.current) {
        ...
        exposureCanvasRef.current.width = EXP_N;
        exposureCanvasRef.current.height = EXP_N;
      }
      const ec = exposureCanvasRef.current;
      const ectx = ec.getContext("2d", { willReadFrequently: true });
      const vw = video.videoWidth, vh = video.videoHeight;
      if (!vw || !vh) return prev;

      // Sample a box roughly 2.5x the iris radius, centred on the iris
      const half = Math.max(8, nrad * vw * 2.5);
      const sx = Math.max(0, Math.min(vw - 1, ncx * vw - half));
      const sy = Math.max(0, Math.min(vh - 1, ncy * vh - half));
      const sw = Math.min(vw - sx, half * 2);
      const sh = Math.min(vh - sy, half * 2);
      if (sw < 4 || sh < 4) return prev;

      try {
        ectx.drawImage(video, sx, sy, sw, sh, 0, 0, EXP_N, EXP_N);
        const d = ectx.getImageData(0, 0, EXP_N, EXP_N).data;
    ...
    const loop = useCallback(() => {
      ...
      if (!res.faceLandmarks?.length) {
        ...
        if (dist > ALIGN_TOL * 1.6) {
          ...
          if (window.__irisDebug) {
            console.log(`[gate] aligned=${alignedFor.toFixed(0)}ms exp.ok=${exp.ok} mean=${(exp.mean||0).toFixed(1)} clip=${((exp.clipped||0)*100).toFixed(0)}% hold=${holdStartRef.current ? (performance.now()-holdStartRef.current).toFixed(0)+"ms" : "null"}`);
          }
          // Exposure is ADVISORY only. It never blocks the capture — it just delays
          // it briefly with a hint, and gives up after 1.5s. A mediocre frame beats
          // a scan that never fires.
          const badExposure = !exp.ok && alignedFor < 1500;
          if (badExposure) {
            setHudState("aligning");
            if (exp.mean < 28) setStatus("More light needed");
            else if (exp.mean > 232 || exp.clipped >= 0.55) setStatus("Too bright \u2014 turn away");
            else setStatus("Hold steady");
          } else {
            setHudState("locking");
          ...
          if (performance.now() - holdStartRef.current >= HOLD_MS) {
            if (window.__irisDebug) console.log("[gate] CAPTURE FIRING");
    ...
    const capture = useCallback((ncx, ncy, nrad) => {
      if (window.__irisDebug) console.log("[capture] entered", { ncx, ncy, nrad });
      ...
      const dominantHue = pk * 5 + 2.5;
      const meanSat = wCount > 0 ? wsSum / wCount : 0;
      const meanLight = wCount > 0 ? wlSum / wCount : 0;
      const colorLabel = classifyColor(dominantHue, meanSat, meanLight);

      // Extra evidence for Claude Vision: mean sclera-corrected RGB of the iris ring,
      // plus channel dominance. Blue eyes show B > R even under warm light once corrected.
      let rSum = 0, gSum = 0, bSum = 0, rgbCount = 0;
      const hueHist = new Float32Array(12); // 12 coarse bins of 30 deg for a compact summary
      for (let y = 0; y < CROP; y += 2) for (let x = 0; x < CROP; x += 2) {
        const dx = x - cxp, dy = y - cyp, dist = Math.sqrt(dx * dx + dy * dy);
        if (dist >= ringIn && dist <= ringOut) {
          const i = (y * CROP + x) * 4;
          const r = Math.min(255, d[i] * wr), g = Math.min(255, d[i + 1] * wg), b = Math.min(255, d[i + 2] * wb);
          rSum += r; gSum += g; bSum += b; rgbCount++;
          const [h, s] = rgbToHsl(r, g, b);
          if (s > 0.07) hueHist[Math.floor(h / 30) % 12] += s * s;
        }
      }
      const meanR = rgbCount ? rSum / rgbCount : 0;
      const meanG = rgbCount ? gSum / rgbCount : 0;
      const meanB = rgbCount ? bSum / rgbCount : 0;
      const histTotal = hueHist.reduce((a, v) => a + v, 0) || 1;
      const hueProfile = Array.from(hueHist).map((v) => Math.round((v / histTotal) * 100));

      const pixelEvidence = {
        dominantHue: Math.round(dominantHue),
        meanSaturation: +meanSat.toFixed(3),
        meanLightness: +meanLight.toFixed(3),
        meanRGB: [Math.round(meanR), Math.round(meanG), Math.round(meanB)],
        blueOverRed: +(meanB - meanR).toFixed(1),
        whiteBalanceGains: [+wr.toFixed(2), +wg.toFixed(2), +wb.toFixed(2)],
        scleraSamples: sn,
        hueProfilePercent: hueProfile,
        pixelGuess: colorLabel,
        frameLuminance: Math.round(exposureRef.current?.mean ?? 0),
        clippedFraction: +(exposureRef.current?.clipped ?? 0).toFixed(3)
      };

      // Tight iris-only crop, upscaled: pupil-to-limbus with nothing else in frame.
      const IRIS_OUT = 384;
      const ic = document.createElement("canvas");
      ic.width = IRIS_OUT; ic.height = IRIS_OUT;
      const ictx = ic.getContext("2d");
      ictx.imageSmoothingEnabled = true;
      ictx.imageSmoothingQuality = "high";
      const srcR = irisR * 1.02;
      ictx.drawImage(c, cxp - srcR, cyp - srcR, srcR * 2, srcR * 2, 0, 0, IRIS_OUT, IRIS_OUT);
      const irisCropUrl = ic.toDataURL("image/jpeg", 0.95);

      const dataUrl = c.toDataURL("image/jpeg", 0.9);
      lastCropRef.current = dataUrl;
      lastColorRef.current = colorLabel;
      lastIrisCropRef.current = irisCropUrl;
      lastEvidenceRef.current = pixelEvidence;
      stopCamera();
      setGenError(null);
      setScreen("processing");
      generate(dataUrl, colorLabel, irisCropUrl, pixelEvidence);
    }, [stopCamera]);
    const generate = useCallback(async (dataUrl, colorLabel, irisCropUrl, pixelEvidence) => {
      ...
      try {
        // Auto-retry the whole generation flow on transient Replicate failures (E9243 etc).
        // The guest never sees these — they just keep watching the processing animation.
        const MAX_TRIES = 3;
        let output = null;
        let finalColor = colorLabel;
        let lastErr = null;

        for (let attempt = 1; attempt <= MAX_TRIES && !output; attempt++) {
          try {
            const startRes = await fetch("/api/generate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageDataUrl: dataUrl, colorLabel, irisCropUrl, pixelEvidence }) });
            const start = await startRes.json();
            if (!startRes.ok || !start.id) throw new Error(start.error || "Could not start generation");
            finalColor = start.confirmedColor || colorLabel;
            ...
            for (let i = 0; i < 60; i++) {
              ...
              if (p.status === "failed" || p.status === "canceled") {
                if (p.retryable && attempt < MAX_TRIES) {
                  console.warn(`[generate] transient failure, retrying (attempt ${attempt + 1}/${MAX_TRIES})`);
                  transientFail = true;
                  break;
                }
            ...
            if (transientFail) { await new Promise((r) => setTimeout(r, 1200)); continue; }
            if (!output) throw new Error("Generation timed out. Please try again.");
          } catch (err) {
            lastErr = err;
            const msg = String(err.message || err);
            const isTransient = /E9243|E6716|E8765|E1000|Could not start/.test(msg);
            if (isTransient && attempt < MAX_TRIES) {
              console.warn(`[generate] retrying after error (attempt ${attempt + 1}/${MAX_TRIES}):`, msg);
        ...
        clearInterval(t);
        if (!output) throw (lastErr || new Error("Generation failed. Please try again."));
        const item = {
          url: output,
          color: finalColor,
