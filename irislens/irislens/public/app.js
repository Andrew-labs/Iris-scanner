...
function IrisLens() {
      ...
      try {
        ...
        for (const entry of manifest) {
          ...
          try {
            ...
            const det = await fa.detectSingleFace(img, new fa.SsdMobilenetv1Options({
              minConfidence: 0.4
  ...
  const capture = useCallback((ncx, ncy, nrad) => {
    ...
    generate(dataUrl, colorLabel, fc);
  }, [stopCamera]);
  ...
  const matchFace = useCallback(async faceCanvas => {
    ...
    const nextFreeSeat = () => {
      ...
      const allSeats = db.length > 0 ? db.map(e => e.seat).filter(s => s !== "admin") : null;
      if (allSeats && allSeats.length) {
        const free = allSeats.find(s => !takenSeatsRef.current.has(s));
        if (free) {
          takenSeatsRef.current.add(free);
          return free;
        }
      }
    ...
    try {
      const detection = await fa.detectSingleFace(faceCanvas).withFaceLandmarks().withFaceDescriptor();
      if (!detection) return {
        name: null,
        seat: nextFreeSeat()
      };
  ...
  const generate = useCallback(async (dataUrl, colorLabel, faceCanvas) => {
    ...
    try {
      ...
      const facePromise = faceCanvas ? matchFace(faceCanvas) : Promise.resolve({
        name: null,
        seat: "—"
      });
      ...
      const item = {
        ...
        seat: faceMatch.seat,
        name: faceMatch.name,
        // null if no match — not shown to guest
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onClick: () => {
      setGenError(null);
      lastCropRef.current ? generate(lastCropRef.current, lastColorRef.current, lastFaceCropRef.current) : setScreen("capture");
    }
  }, "Try Again"), /*#__PURE__*/React.createElement("button", {
    className: "btn-ghost",
    style: {
  ...
  }, result.seat === "admin" ? "ADMIN" : result.seat))), /*#__PURE__*/React.createElement("div", {
    style: {
      width: "100%",
      maxWidth: 420,
  ...
  }, expanded.seat === "admin" ? "Admin" : `Seat ${expanded.seat}`)), showExplainer && /*#__PURE__*/React.createElement("div", {
    style: {
      position: "fixed",
      inset: 0,
